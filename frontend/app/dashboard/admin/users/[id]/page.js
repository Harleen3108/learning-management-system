...
function Badge({ status, label }) {
    ...
    return (
        <span className={clsx(
            "px-3 py-1 rounded-xl text-[9px] font-semibold uppercase tracking-widest border",
        ...
        return (
            <AdminLayout>
                <div className="flex flex-col items-center justify-center min-h-[60vh] gap-4">
                    ...
                    <p className="text-slate-400 font-semibold text-xs uppercase tracking-widest animate-pulse">Analyzing User Data...</p>
                </div>
            </AdminLayout>
        );
    }
        ...
        return (
            <AdminLayout>
                <div className="flex flex-col items-center justify-center min-h-[60vh] gap-4">
                    ...
                    <p className="text-slate-500 font-semibold">User profile not found</p>
                    <button onClick={() => router.back()} className="text-[#071739] font-semibold text-xs uppercase tracking-widest hover:underline">← Go Back</button>
                </div>
            </AdminLayout>
        );
    }
        ...
        <AdminLayout>
            <AnimatePresence>
                {toast && (
                    <motion.div
                        ...
                        className={`fixed top-0 right-1/2 z-[999] px-6 py-3 rounded-2xl text-white text-sm font-semibold shadow-2xl translate-x-1/2
            ...
            <div className="space-y-8 max-w-7xl mx-auto pb-20">
                ...
                <div className="flex items-center gap-4">
                    ...
                    <div>
                        <p className="text-[10px] font-semibold text-slate-400 uppercase tracking-widest">User Management / Profile</p>
                    ...
                    <div className="flex flex-col md:flex-row md:items-center gap-8">
                        ...
                        <div className="flex-1 min-w-0">
                            <div className="flex flex-wrap items-start justify-between gap-4">
                                <div>
                                    <h2 className="text-3xl font-semibold text-slate-800 tracking-tight leading-tight">{profile.name}</h2>
                                    <div className="flex items-center gap-2 mt-1">
                                        <Badge status={profile.role} />
                                        <span className="text-xs font-semibold text-slate-400">• Joined {date(profile.createdAt)}</span>
                                    </div>
                                </div>
                                <div className="flex gap-2">
                                    <button 
                                        ...
                                        className={clsx(
                                            "px-6 py-3 rounded-2xl text-[10px] font-semibold uppercase tracking-widest transition-all",
                                            profile.isActive 
                ...
                <div className="flex gap-2 overflow-x-auto no-scrollbar pb-2">
                    {tabs.map((t, i) => (
                        <button
                            ...
                            className={clsx(
                                "flex items-center gap-2 px-6 py-3.5 rounded-2xl text-[10px] font-semibold uppercase tracking-widest transition-all shrink-0 border",
                                activeTab === t.key 
                                    ...
                                    <div className="bg-white p-8 rounded-[2.5rem] border border-slate-100 shadow-sm">
                                        ...
                                        <div className="space-y-4">
                                            {enrollments.slice(0, 3).map((e, i) => (
                                                <div key={i} className="flex items-center justify-between p-5 bg-slate-50 rounded-[1.5rem] border border-slate-100 hover:border-[#071739]/20 transition-all group">
                                                    <div className="flex items-center gap-4">
                                                        ...
                                                        <div>
                                                            <p className="text-sm font-semibold text-slate-700">{e.course?.title}</p>
                                                            <p className="text-[10px] font-semibold text-slate-400 mt-0.5">by {e.course?.instructor?.name}</p>
                                                        </div>
                                                    </div>
                                                    <div className="text-right">
                                                        <Badge status={e.status} />
                                                        <p className="text-[10px] font-semibold text-slate-400 mt-2">{date(e.enrolledAt)}</p>
                                                    </div>
                                                </div>
                                            ))}
                                            {enrollments.length === 0 && <p className="text-center py-10 text-slate-400 font-semibold">No active enrollments</p>}
                                        </div>
                                    </div>
                                        ...
                                        <div className="space-y-4">
                                            {activityLogs.slice(0, 5).map((log, i) => (
                                                <div key={i} className="flex gap-4 items-start relative pb-6 last:pb-0">
                                                    ...
                                                    <div>
                                                        <p className="text-xs font-semibold text-slate-700 uppercase tracking-widest">{log.action}</p>
                                                        <p className="text-sm text-slate-500 mt-1">{log.details}</p>
                                                        <p className="text-[10px] font-semibold text-slate-400 mt-1">{date(log.timestamp)} at {time(log.timestamp)}</p>
                                                    </div>
                                                </div>
                                            ))}
                                        </div>
                                        ...
                                        <div className="space-y-3">
                                            {certificates.map((cert, i) => (
                                                <div key={i} className="flex items-center gap-3 p-4 bg-emerald-50/50 rounded-2xl border border-emerald-100">
                                                    ...
                                                    <div>
                                                        <p className="text-[11px] font-semibold text-emerald-800">{cert.course?.title}</p>
                                                        <p className="text-[9px] font-semibold text-emerald-600 mt-0.5">{date(cert.issueDate)}</p>
                                                    </div>
                                                </div>
                                            ))}
                                            {certificates.length === 0 && <p className="text-center py-6 text-slate-300 text-xs font-semibold">No certificates issued</p>}
                                        </div>
                                    </div>
                                            ...
                                            {tickets.filter(t => t.status !== 'Resolved').map((t, i) => (
                                                <div key={i} className="p-4 bg-slate-50 rounded-2xl border border-slate-100">
                                                    <div className="flex justify-between mb-1">
                                                        <span className="text-[10px] font-semibold text-slate-400 uppercase tracking-widest">{t.category}</span>
                                                        <span className="text-[9px] font-semibold text-[#071739] uppercase">{t.status}</span>
                                                    </div>
                                                    <p className="text-xs font-semibold text-slate-700 line-clamp-1">{t.subject}</p>
                                                </div>
                                            ))}
                                            {tickets.filter(t => t.status !== 'Resolved').length === 0 && <p className="text-center py-6 text-slate-300 text-xs font-semibold">No active tickets</p>}
                                        </div>
                                    </div>
                                </div>
                            </div>
                                    ...
                                    <table className="w-full text-left">
                                        <thead>
                                            <tr className="bg-slate-50/80">
                                                <th className="px-8 py-4 text-[10px] font-semibold text-slate-400 uppercase tracking-widest">Course</th>
                                                <th className="px-8 py-4 text-[10px] font-semibold text-slate-400 uppercase tracking-widest">Instructor</th>
                                                <th className="px-8 py-4 text-[10px] font-semibold text-slate-400 uppercase tracking-widest">Progress</th>
                                                <th className="px-8 py-4 text-[10px] font-semibold text-slate-400 uppercase tracking-widest">Enrolled At</th>
                                                <th className="px-8 py-4 text-[10px] font-semibold text-slate-400 uppercase tracking-widest">Status</th>
                                            </tr>
                                        </thead>
                                        <tbody className="divide-y divide-slate-50">
                                            {enrollments.map((e, i) => (
                                                <tr key={i} className="hover:bg-slate-50/50 transition-all">
                                                    ...
                                                    <td className="px-8 py-6 text-sm font-semibold text-slate-500">{e.course?.instructor?.name}</td>
                                                    <td className="px-8 py-6">
                                                        <div className="flex items-center gap-3">
                                                            <div className="flex-1 h-1.5 bg-slate-100 rounded-full overflow-hidden">
                                                                <div className="h-full bg-primary" style={{ width: e.status === 'completed' ? '100%' : '35%' }} />
                                                            </div>
                                                            <span className="text-[10px] font-semibold text-slate-400">{e.status === 'completed' ? '100%' : '35%'}</span>
                                                        </div>
                                                    </td>
                                                    <td className="px-8 py-6 text-sm font-semibold text-slate-500">{date(e.enrolledAt)}</td>
                                    ...
                                    <div className="space-y-4">
                                        {quizPerformance.results.map((r, i) => (
                                            <div key={i} className="flex items-center justify-between p-5 bg-slate-50 rounded-2xl border border-slate-100">
                                                <div>
                                                    <p className="text-sm font-semibold text-slate-700">{r.quiz?.title}</p>
                                                    <p className="text-[10px] font-semibold text-slate-400 mt-1 uppercase tracking-widest">{r.quiz?.module?.title}</p>
                                                </div>
                                                <div className="flex items-center gap-8">
                                                    <div className="text-center">
                                                        <p className={clsx("text-lg font-semibold", r.passed ? 'text-emerald-500' : 'text-rose-500')}>{r.score}%</p>
                                                    ...
                                                    <div className="text-right">
                                                        ...
                                                        <p className="text-[10px] font-semibold text-slate-400 mt-2">{date(r.attemptedAt)}</p>
                                                    </div>
                                                </div>
                                            </div>
                                        ))}
                                        {quizPerformance.results.length === 0 && <p className="text-center py-20 text-slate-300 font-semibold uppercase tracking-widest text-xs">No quiz data available</p>}
                                    </div>
                                </div>
                            </div>
                        )}
                            ...
                            <div className="bg-white rounded-[2.5rem] border border-slate-100 shadow-sm overflow-hidden">
                                ...
                                <div className="overflow-x-auto">
                                    <table className="w-full text-left">
                                        <thead>
                                            <tr className="bg-slate-50/80">
                                                <th className="px-8 py-4 text-[10px] font-semibold text-slate-400 uppercase tracking-widest">Course / Item</th>
                                                <th className="px-8 py-4 text-[10px] font-semibold text-slate-400 uppercase tracking-widest">Txn ID</th>
                                        ...
                                        <tbody className="divide-y divide-slate-50">
                                            {payments.map((p, i) => (
                                                <tr key={i} className="hover:bg-slate-50/50 transition-all font-semibold text-sm">
                                                    <td className="px-8 py-6 text-slate-700">{p.course}</td>
                                                    <td className="px-8 py-6 text-[#071739] font-mono text-[10px]">{p.transactionId}</td>
                                        ...
                                        <div key={i} className="group relative flex gap-8 pl-6 pb-10 last:pb-0">
                                            ...
                                            <div className="flex-1 bg-slate-50 p-6 rounded-3xl border border-slate-100 group-hover:bg-white group-hover:border-blue-100 group-hover:shadow-lg group-hover:shadow-blue-50 transition-all">
                                                <div className="flex justify-between items-start mb-2">
                                                    <span className="px-3 py-1 bg-white text-[10px] font-semibold text-primary rounded-xl border border-primary/5 uppercase tracking-widest shadow-sm">
                                                        {log.action}
                                                    </span>
                                                    <span className="text-[10px] font-semibold text-slate-400">{date(log.timestamp)} at {time(log.timestamp)}</span>
                                                </div>
                                                <p className="text-sm font-semibold text-slate-700">{log.details}</p>
                                                {log.ipAddress && <p className="text-[9px] font-mono text-slate-300 mt-2">IP: {log.ipAddress}</p>}
                                            </div>
                                        </div>
                                    ))}
                                    {activityLogs.length === 0 && <p className="text-center py-20 text-slate-300 text-sm font-semibold uppercase tracking-widest">No audit trails found</p>}
                                </div>
                            </div>
                        )}
                                            ...
                                            <div key={i} className="flex items-center justify-between p-5 bg-blue-50/50 rounded-2xl border border-blue-100">
                                                <div className="flex items-center gap-3">
                                                    ...
                                                    <div>
                                                        ...
                                                        <p className="text-[10px] font-semibold text-slate-400">{p.email}</p>
                                                    </div>
                                                </div>
                                                <button onClick={() => router.push(`/dashboard/admin/users/${p._id}`)} className="p-2 text-primary/40 hover:text-primary transition-all">
                                                    <Eye size={16} />
                                                </button>
                                            </div>
                                        ))}
                                        {linkedAccounts.parents.length === 0 && <p className="text-center py-20 text-slate-300 text-xs font-semibold uppercase tracking-widest">No parents linked</p>}
                                    </div>
                                </div>
                                <div className="bg-white p-8 rounded-[2.5rem] border border-slate-100 shadow-sm">
                                    ...
                                    <div className="space-y-4">
                                            ...
                                            <div key={i} className="flex items-center justify-between p-5 bg-violet-50/50 rounded-2xl border border-violet-100">
                                                <div className="flex items-center gap-3">
                                                    ...
                                                    <div>
                                                        ...
                                                        <p className="text-[10px] font-semibold text-slate-400">{s.email}</p>
                                                    </div>
                                                </div>
                                        ...
                                        {linkedAccounts.students.length === 0 && <p className="text-center py-20 text-slate-300 text-xs font-semibold uppercase tracking-widest">No children linked</p>}
                                    </div>
                                </div>
                            </div>
                        )}
                                ...
                                <div className="bg-white p-10 rounded-[3rem] border border-slate-100 shadow-sm">
                                    ...
                                    <div className="space-y-6 mt-8">
                                        <div className="p-6 bg-slate-50 rounded-3xl border border-slate-100 flex flex-col items-center text-center">
                                            ...
                                            <h4 className="text-lg font-semibold text-slate-800">Forced Password Reset</h4>
                                            <p className="text-xs text-slate-400 font-semibold max-w-[240px] mt-2 mb-6">Manually override user password for recovery or security breach protocol.</p>
                                            <button 
                                                onClick={handlePasswordReset}
                                                disabled={actionLoading}
                                                className="w-full py-4 bg-slate-800 text-white rounded-2xl text-[10px] font-semibold uppercase tracking-widest hover:bg-primary transition-all shadow-xl shadow-slate-200"
                                            >
                                        ...
                                        <div className="p-6 bg-rose-50/50 rounded-3xl border border-rose-100 flex flex-col items-center text-center">
                                            ...
                                            <h4 className="text-lg font-semibold text-rose-600">Account Deactivation</h4>
                                            <p className="text-xs text-slate-400 font-semibold max-w-[240px] mt-2 mb-6">Soft delete user account. All data remains but access is permanently revoked.</p>
                                            <button 
                                                onClick={handleDelete}
                                                disabled={actionLoading}
                                                className="w-full py-4 bg-rose-500 text-white rounded-2xl text-[10px] font-semibold uppercase tracking-widest hover:bg-rose-600 transition-all shadow-xl shadow-rose-100"
                                            >
                                                Deactivate Account
                                            </button>
                                        </div>
                                    ...
                                    <div className="space-y-4 pt-4">
                                        {[
                                            ...
                                            <div key={i} className="flex justify-between items-center p-5 bg-slate-50/50 rounded-2xl">
                                                ...
                                                <span className={clsx("text-[11px] font-semibold tracking-widest", item.color)}>{item.val}</span>
                                            </div>
                                        ))}
                                        <div className="mt-10 p-8 bg-primary rounded-[2.5rem] text-white overflow-hidden relative group">
                                            ...
                                            <h5 className="text-lg font-semibold relative z-10 leading-tight">Identity Verification</h5>
                                            <p className="text-[10px] font-semibold text-white/70 mt-2 relative z-10 leading-relaxed">System-wide unique identifier and biometric trace status for platform security.</p>
